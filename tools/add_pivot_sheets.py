...
enhanced_versions = sorted(OUTPUT_DIR.glob("饮料赛道_小红书_增强版_*.xlsx"))
if not enhanced_versions:
    print("❌ 找不到增强版 Excel！请先运行 enhance_excel.py")
    sys.exit(1)
SRC = enhanced_versions[-1]
...
GROUP_ORDER = list(GROUP_ORDER)  # 使用 config 中的排序
# 动态配色：根据组数循环使用预定义颜色
_COLOR_POOL = [LIGHT_ORANGE, LIGHT_GREEN, LIGHT_BLUE, ACCENT_FILL]
GROUP_COLORS = {g: _COLOR_POOL[i % len(_COLOR_POOL)] for i, g in enumerate(GROUP_ORDER)}
...
all_keywords = []
for g in GROUP_ORDER:
    g_kws = [k for k, v in KW_TO_GROUP.items() if v == g]
    for kw in g_kws:
